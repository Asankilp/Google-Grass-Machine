...
if choice == 1:
    ...
    with open("./Input.txt",encoding="utf-8") as Inputs:
        for Input in Inputs:
            text = Input.split("\n")
            for mubiao in lang:
                text[0] = translator.translate(text[0],dest=mubiao).text
        print(text[0])
        Out.append(text[0]+"\n")
    with open("./Output.txt",encoding="utf-8",mode="w") as Output:Output.writelines(Out)
    print("已将翻译结果存储至Output.txt。")
#elif choice == 3:
  #  pass
   # ##TODO:制作GUI,大咕等周末吧
else:
